 ...
 *     at a blob: URL holding the personalised copy. When iOS Safari
 ...
 * and a graceful no-op when localStorage is unavailable, when the
 * manifest fetch fails, or when blob URLs are blocked. The worst-case
 * fallback in any failure mode is today's behaviour: re-sign-in after
 * install.
 */
...
(function () {
  ...
  async function injectSessionIntoManifest() {
    ...
    // Resolve against the original static manifest, not whatever blob
    // we may have already swapped in.
    ...
    manifest.start_url = "/#" + HASH_PARAM + "=" + encodeURIComponent(token);
    ...
    if (lastManifestBlobUrl) {
      try { URL.revokeObjectURL(lastManifestBlobUrl); } catch { /* ignore */ }
    }
