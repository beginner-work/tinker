 ...
 *
 * Bridges the client-owned blobs (essays, drafts, earths, taxonomy,
 * tree) to /api/user-data/<kind>. On boot: when an auth token is
 * available, fetch each blob and overwrite the corresponding
 * localStorage key so the consumer modules (earths.js, heatmap.js,
 * renderer.js, tree.js) see the server state on their next read. On
 ...
 *                Server kind name is still "seeds" (v0.100 storage
 *                shape) — the rename happened client-side; the server
 *                blob shape is unchanged.
 *   - taxonomy → "tinker.taxonomy.v1" (object)
 *   - tree     → "tinker.tree.v1"     (object, the cached clustering
 *                response — see api/cluster)
 *
...
(function () {
  ...
  // The server kind is still "seeds" (storage was provisioned under
  // that name in v0.100). The client-side rename to "earths" is purely
  // semantic — the blob shape and the row in TinkerUserData are
  // untouched. If a future migration moves the server row, do it in
  // one PR with a backfill, not a rename here.
  const KIND_EARTHS = "seeds";
  ...
  //
  // Wait for DOMContentLoaded so consumer modules (earths.js, tree.js,
