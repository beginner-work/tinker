 ...
 * transactions) to /api/user-data/<kind>. On boot: when an auth token
 * is available, fetch each blob and overwrite the corresponding
 * localStorage key so the consumer modules (seeds.js, heatmap.js,
 * renderer.js, transactions.js) see the server state on their next
 ...
 *   - transactions → "tinker.transactions.v1" (array)
 *
...
(function () {
  ...
  const KIND_TRANSACTIONS = "transactions";
  ...
  const LS_TRANSACTIONS = "tinker.transactions.v1";
  ...
  function applyTransactionsFromServer(data) {
    if (!Array.isArray(data)) return false;
    setLs(LS_TRANSACTIONS, JSON.stringify(data));
    return true;
  }
  ...
  const api = {
    ...
    pushTransactions() {
      schedulePush(KIND_TRANSACTIONS, () => getLsJson(LS_TRANSACTIONS, []));
    },
    ...
    flush() {
      ...
      if (kinds.includes(KIND_TRANSACTIONS)) pushKind(KIND_TRANSACTIONS, getLsJson(LS_TRANSACTIONS, []));
    },
  };

  async function hydrate() {
    if (!token()) return;
    const [essays, drafts, seeds, taxonomy, transactions] = await Promise.all([
      fetchKind(KIND_ESSAYS),
      fetchKind(KIND_DRAFTS),
      fetchKind(KIND_SEEDS),
      fetchKind(KIND_TAXONOMY),
      fetchKind(KIND_TRANSACTIONS),
    ]);
    let changed = false;
    ...
    if (applyTransactionsFromServer(transactions)) changed = true;
    if (changed) {
