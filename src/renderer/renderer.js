...
(() => {
  ...
  const store = {
    ...
    publish(draft, stitched) {
      ...
      const essay = {
        ...
        kind: "essay",
        // Carry earth through so future clustering can place this
        // writing under the right place in the sidebar tree.
      ...
      renderSidebar();
      renderHome();
      // Land on the read view of the just-published essay. The sidebar
      // tree will refresh on the next clustering pass (triggered by
      // writing.js's close flow) and the new essay will land under
      // its earth then.
      showRead(essay);
      // Kick a tree refresh so the new essay is reflected on the
      // sidebar without waiting for the next session close.
    ...
    // earth so the next clustering pass places it under the right
    // place in the sidebar tree.
    publishStatus({ body, earth }) {
      ...
      essays = [essay, ...essays];
      saveEssays(essays);
      // Make sure the earth exists in the explicit list. add() is a
  ...
  // ── Rendering ───────────────────────────────────────────────────────
  // Sidebar's drafts+essays list is gone — the three-tier earth tree
  // owns the sidebar (see #sidebar-tree). Each earth row reveals
  // AI-derived seed clusters when tapped; each seed reveals its
  // growth vectors. Kept as a no-op so existing call sites compile.
  ...
  function renderHome() {
    ...
    if (window.tinkerTree && typeof window.tinkerTree.render === "function") {
      window.tinkerTree.render(treeEl);
    }
  }
  ...
  // Used by the welcome grid and the sidebar tree: open a fresh draft
  ...
  // Used by the sidebar tree's growth-vector taps to route to the
  // right surface — a published essay or an in-progress draft.
