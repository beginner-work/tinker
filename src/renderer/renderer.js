...
(() => {
  ...
  const store = {
    ...
    publish(draft, stitched) {
      ...
      const essay = {
        ...
        kind: "essay",
        // Carry the earth through so the clustering classifier can
        // keep reading the writing content after publish.
      ...
      renderSidebar();
      renderHome();
      // Refresh the clustering tree on the side now that a new essay
      // exists. tree.js handles the cache + server round-trip; we
      // just kick it off and don't wait.
      if (window.tinkerTree && typeof window.tinkerTree.refresh === "function") {
        window.tinkerTree.refresh();
      }
      // Drop the founder back into the read view on the published
      // essay. The category-feed routing is intentionally gone —
      // tap-down-to-a-writing now goes through the sidebar tree.
      showRead(essay);
      return essay;
    },
    // Short-form post: typed straight into a category-feed textarea
    // (legacy surface, still wired so existing flows keep working).
    // Skips the interview flow and the stitching/verification logic
    // — the body is exactly what the founder typed. Tied to an Earth
    // so the clustering tree knows where to surface it.
  ...
  function showCategoryFeed(categoryKey, originatingEarth) {
    ...
    // Prefer the earth the founder just tapped. Fall back to the most-
    // recently-touched earth in this category so the composer still
    // has somewhere to attach a status.
  ...
  // ── Rendering ───────────────────────────────────────────────────────
  // Sidebar tree is owned by tree.js — it renders into #sidebar-tree
  // on hydrate and after every writing-session close. renderSidebar
  // and renderHome stay as no-ops so existing call sites compile.
  ...
  // anchored to that Earth. "Somewhere else" reveals a small input
  ...
  // Re-render the sidebar tree whenever earths change.
  ...
  // Used by the welcome grid (and any sidebar entry point): open a
  // fresh draft pre-filled with scene context so the founder jumps
  // straight into mood-tuned reflection.
  window.tinkerNewSession = (preset) => newDraft({ activate: true, preset: preset || null });

  // Used by the sidebar tree when a Growth vector resolves to a
  // published essay or an in-progress draft — tap routes to the right
  // surface instead of always spawning a new session.
