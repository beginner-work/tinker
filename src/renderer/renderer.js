...
(() => {
  ...
  const readView = $("#read");
  const sidebarTreeEl = $("#sidebar-tree");
  ...
  const store = {
    ...
    publish(draft, stitched) {
      ...
      const essay = {
        ...
        kind: "essay",
        // Carry Earth through so the sidebar tree's clustering step
      ...
      renderSidebar();
      renderHome();
      // Trigger a re-cluster: a freshly published essay is the strongest
      // signal that the tree needs to be refreshed. Fire and forget —
      // the renderer will pick up the new tree via the cache update.
      ...
      const leafKey = (window.tinkerHeatmap && typeof window.tinkerHeatmap.getCategoryKeyForEarth === "function")
        ? window.tinkerHeatmap.getCategoryKeyForEarth(essay.earth)
        : null;
    ...
    // Earth so the clustering step keeps placing it in the same place
    // the founder posted from.
  ...
  // ── Rendering ───────────────────────────────────────────────────────
  // Sidebar's drafts+essays list is gone — the three-tier tree
  // (Earth → Seed → Growth vector) now owns the sidebar slot between
  // .sidebar__top and .sidebar__account. Kept as a no-op so existing
  // call sites compile.
  ...
  // Re-render the sidebar tree from whatever data tinkerTree has
  // cached. With no Earths-yet-with-Seeds, the tree module hides
  // the <nav> entirely and the brand block sits directly above the
  // Account block.
  function renderHome() {
    if (!sidebarTreeEl) return;
    if (window.tinkerTree && typeof window.tinkerTree.render === "function") {
      window.tinkerTree.render(sidebarTreeEl);
    }
  }
  ...
  // anchored at that Earth. "Somewhere else" reveals a small input
  ...
  // Re-render the sidebar tree whenever Earths change. The tree only
  // reflects clustered data, so this re-render is mostly cosmetic
  // (it re-evaluates which rows to show against the cached cluster);
  // the actual cluster recompute fires on writing close.
