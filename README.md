...
- The in-app email composer (profile menu → "Send an email",
  `api/email/send.js`) needs **no env vars here**. The function relays to the
  beginner mcp Worker's `POST /email/send` (canonical URL
  `https://beginner-mcp.tyler-lindow.workers.dev`, hardcoded default;
  `BEGINNER_MCP_URL` overrides), forwarding the signed-in founder's own
  Stytch session token as the bearer — the Worker validates it against the
  same Stytch project (its `STYTCH_PROJECT_ID`/`STYTCH_SECRET` secrets), so
  no cross-cloud server secret exists. `BEGINNER_MCP_TOKEN` (or
  `CLOUDFLARE_API_TOKEN`) is an optional override for a static server
  bearer. Sending is from the founder's `beginner.work` address via
  Cloudflare Email Routing; inherited constraint: delivery only to
  **verified destination addresses** on the Cloudflare account.
