...
`get_career_record` and `check_text` on `POST /api/mcp` read that same key for the connector user. Neither tool takes a user id. Both use the read-only token. `get_career_record` returns verified facts and rules with ids. Proposed facts are returned on `unverified_facts` with `unverified: true` and must not be used as facts. Rejected facts are omitted. If the store cannot be reached, the tool returns `Career record is unavailable right now.` and no record. `check_text` lists claims with verdict `pass`, `mismatch`, `unsupported`, or `needs_claire`. `ready` is true only when every claim passes. Numbers, dates, titles, and employer names are compared in code. The model only finds claim text. Kinds, rules, and the seeds live in `src/renderer/career/catalog.js`, which the page and the server both read. Preview logs redact career bodies. The app does not log resume text, facts, or the checked draft, and it does not put the REST URL or either token in an error or a response.
