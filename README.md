...
`/career` stores one career record per signed-in person in the same Upstash Redis, not Postgres. The key is `career:<user id>`. The id comes from the verified session. The value is JSON with `facts` and `rules`. A fact has an id, a kind, a value, a source document and excerpt, a status (`proposed`, `verified`, or `rejected`), and `updated_at`. A metric can also carry `baseline` and `mechanism`. A rule has an id, a field, the rule text, a machine-usable form, and status `verified`.

The first visit seeds four proposed facts (the L7 people-leadership outcome, the mechanism and baseline behind the $500K figure, the mechanism and baseline behind the 99.9% figure, and the combined 1-to-9 plus US/Canada/Poland same-team claim) and the six verified answer rules. Upload accepts a PDF or markdown resume. Proposed facts come back with the source excerpt. Confirm, edit, reject, and restore are browser actions on `POST /api/career?action=fact`. Rejected facts stay in a collapsed Rejected section, each with its source excerpt and a Restore button that returns the fact to proposed. Nothing is verified without a confirm click. The upload itself is not stored.

`GET /api/career` uses `KV_REST_API_READ_ONLY_TOKEN`, then the write token only to create the seed when the key is missing. `POST /api/career?action=extract` and `POST /api/career?action=fact` use the write token. A bearer that starts with `mcp_` is rejected on every career route before Stytch. If the store is missing or cannot be reached, the route returns 503 with `Career record is unavailable right now.` and writes nothing.

`get_career_record` and `check_text` on `POST /api/mcp` read that same key for the connector user. Neither tool takes a user id. Both use the read-only token. `get_career_record` returns verified facts and rules with ids. Proposed facts are returned on `unverified_facts` with `unverified: true` and must not be used as facts. Rejected facts are omitted. Restoring a fact returns it to proposed, and the tool lists it again on `unverified_facts` with `unverified: true`. If the store cannot be reached, the tool returns `Career record is unavailable right now.` and no record. `check_text` lists claims with verdict `pass`, `mismatch`, `unsupported`, or `needs_claire`. `ready` is true only when every claim passes, and false when the draft is empty or no claims were found. Numbers, dates, titles, and employer names are compared in code. A title passes only with the employer on that same fact. Baseline and mechanism claims compare the amount, percentage, and baseline. The model only finds claim text. Kinds, rules, and the seeds live in `src/renderer/career/catalog.js`, which the page and the server both read. Preview logs redact career bodies. The app does not log resume text, facts, or the checked draft, and it does not put the REST URL or either token in an error or a response.
