...
Inside the tinker app, the deck is generated by the Anthropic API — call `window.tinker.contentHarnessGenerate(intake)`. The intake shape is:

```js
{
  freeText: {
    companyName, mission, whatYouDo, stage, proofPoints,
    investorContext, // optional — name + firm
    askAmount        // amount + use of funds
  },
  decisions: {
    voice,        // "Confident" | "Warm" | "Technical" | "Visionary"
    archetype,    // "Pre-seed angel" | "Seed VC" | "Series A VC" | "Strategic / Growth"
    topConcern,   // "Team" | "Market" | "Traction" | "Defensibility" | "Capital efficiency"
    depth         // "Short" | "Standard" | "Long"
  },
  followUps: {    // one per axis from step 3b; omit if an axis was skipped
    voice, archetype, topConcern, depth
  }
}
```

The handler returns `{ markdown, usage }`. Write the markdown to `pitch-deck.md` in the working directory verbatim — do not re-author or re-format it. The system prompt on the API side encodes the slide structure, voice rules, archetype reweighting, top-concern emphasis, and the `[needs: ...]` placeholder convention; the model owns those decisions.

Outside the tinker app (running this skill in plain Claude Code without the IPC bridge), fall back to writing the deck directly. Use the same format:

- A one-paragraph `## Shape note` at the top recording every intake choice
