...
test("controller starts synchronously within the caller's gesture", () => {
  // Safari only allows SpeechRecognition.start() inside the click gesture,
  // so start() must begin recognition synchronously — no awaiting.
  let rec;
  const ctrl = createVoiceController({
    createRecognition: () => (rec = new FakeRecognition()),
  });
  ctrl.start();
  assert.ok(rec, "recognition should be created synchronously");
  assert.equal(rec.started, true);
  assert.equal(ctrl.state, "listening");
});
